...
from unitycatalog.types import TableListResponse, SchemaListResponse, TableInfo, SchemaInfo

from dunky.unity import uc_table_exists, uc_schema_exists, uc_get_storage_credentials, create_table_if_not_exists


def test_uc_table_exists_returns_true(mocker):
    mock_client = mocker.Mock(spec=Unitycatalog)
    mock_client.tables = mocker.Mock()
    mock_client.tables.list.return_value = TableListResponse(tables=[TableInfo(name="table_name")])
...
def test_uc_schema_exists_returns_true(mocker):
    ...
    mock_client.schemas.list.return_value = SchemaListResponse(schemas=[SchemaInfo(name="schema_name")])
...
def test_uc_get_storage_credentials_returns_empty(mocker):
    mock_client = mocker.Mock(spec=Unitycatalog)
    mock_client.tables = mocker.Mock()
    mock_client.tables.list.return_value = TableListResponse(tables=[TableInfo(table_id=None)])
    creds = uc_get_storage_credentials(mock_client, "catalog_name", "schema_name", "table_name")
    assert creds == {}
...
def test_create_table_if_not_exists_creates_schema_and_table(mocker):
    ...
    create_table_if_not_exists(
        mock_client, "table_name", "schema_name", "catalog_name", "s3://bucket/path", [], "PARQUET"
    )
