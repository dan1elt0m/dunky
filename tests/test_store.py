...
def test_store_happy_path(mocker):
    ...
    mock_get_storage_credentials = mocker.patch("dunky.store.uc_get_storage_credentials", return_value={})
...
def test_store_partition_key_missing_error(mocker):
    mock_uc_client = mocker.patch("dunky.store.Unitycatalog")
    mock_delta_write = mocker.patch("dunky.store.delta_write", side_effect=PartitionKeyMissingError)
    mock_create_table = mocker.patch("dunky.store.create_table_if_not_exists")
    mock_get_storage_credentials = mocker.patch("dunky.store.uc_get_storage_credentials", return_value={})
...
def test_store_unique_key_missing_error(mocker):
    mock_uc_client = mocker.patch("dunky.store.Unitycatalog")
    mock_delta_write = mocker.patch("dunky.store.delta_write", side_effect=UniqueKeyMissingError)
    mock_create_table = mocker.patch("dunky.store.create_table_if_not_exists")
    mock_get_storage_credentials = mocker.patch("dunky.store.uc_get_storage_credentials", return_value={})
